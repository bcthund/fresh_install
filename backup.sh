...
if [ "$answer" != "${answer#[AaYy]}" ] ;then
    ...
    # Warzone 2100
        ...
        if [ "$answer2" != "${answer2#[Yy]}" ] ;then
            ...
            cmd "sudo rsync -aR --info=progress2 /home/$USER/.local/share/warzone2100-3.3.0 ${TMP_DIR}/root/"
        fi
    if [ "$GOTOSTEP" = true ]; then echo -e "${BLUE}Finished${NC}\n"; exit; fi
    ...
    # Global Shortcuts (KDE) (BACKUP)
        ...
        if [ "$answer2" != "${answer2#[Yy]}" ] ;then
            ...
            cmd "sudo rsync -aR --info=progress2 /home/$USER/.config/khotkeysrc ${TMP_DIR}/root/"
        fi
    if [ "$GOTOSTEP" = true ]; then echo -e "${BLUE}Finished${NC}\n"; exit; fi
